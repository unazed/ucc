...
{
  __extension__ __auto_type file = fopen (filename, mode);
  if (file == 
# 10 "src/io.c" 3 4
             ((void *)0)
# 10 "src/io.c"
...
# 12 "src/io.c" 3 4
     "u" 
# 12 "src/io.c"
     "\033[0m" ": " "\033[1;" "31m" "error: " "\033[0m" "%s: failed to open filename" " (%s)" "\n", "src/io.c", 12, filename, strerror (
# 12 "src/io.c" 3 4
     (*__errno_location ())
# 12 "src/io.c"
     ));
...
# 21 "src/io.c"
  ...
  __extension__ __auto_type filesize = ftell (file);
  fseek (file, 0, 
# 23 "src/io.c" 3 4
                 0
# 23 "src/io.c"
...
{
  return __chk_calloc_impl (__func__, (io_get_filesize (file)), (sizeof (char)));
}

size_t
io_read_file_into_buffer (fptr_t file, uint8_t* const buffer, size_t buffer_size)
{
  if (!(buffer_size <= io_get_filesize (file))) { fprintf ((
# 36 "src/io.c" 3 4
 stderr
# 36 "src/io.c"
 ), "\033[1;" "37m" "%s:%" 
...
# 36 "src/io.c" 3 4
 "u" 
# 36 "src/io.c"
 "\033[0m" ": " "\033[1;" "31m" "error: " "\033[0m" "Writing to the specified buffer would cause an overrun " "file size is %zu bytes, buffer size is %zu bytes" "\n", "src/io.c", 36, io_get_filesize (file), buffer_size); exit (
# 36 "src/io.c" 3 4
 1
# 36 "src/io.c"
 ); }; }
                                                  ...
                                                  ;
  __extension__ __auto_type nbytes_written = fread (buffer, sizeof (char), buffer_size, file);
...
# 42 "src/io.c" 3 4
   "u" 
# 42 "src/io.c"
   "\033[0m" ": " "\033[0;" "34m" "log: " "\033[0m" "read less than expected (expected: %zu, read: %zu)" "\n", "src/io.c", 42, buffer_size, nbytes_written)
                                             ;
...
io_read_file_into_memstream (fptr_t file, const char* const path)
{
  ...
  memstream_t stream = __chk_calloc_impl (__func__, (1), (sizeof (*(memstream_t)
# 53 "src/io.c" 3 4
                      ((void *)0)
# 53 "src/io.c"
                      )));
...
# 64 "src/io.c" 3 4
 "u" 
# 64 "src/io.c"
 "\033[0m" ": " "\033[1;" "36m" "minutiae: " "\033[0m" "freeing memory stream %p" "\n", "src/io.c", 64, ((void *)((uintptr_t)stream & 0xFFFFFFFF)));
  io_close_file (stream->ptr);
  __chk_free_impl (__func__, stream->buffer);
  __chk_free_impl (__func__, stream);
}
...
# 73 "src/io.c" 3 4
 "u" 
# 73 "src/io.c"
 "\033[0m" ": " "\033[1;" "36m" "minutiae: " "\033[0m" "closing file object %p" "\n", "src/io.c", 73, ((void *)((uintptr_t)file & 0xFFFFFFFF)));
  fclose (file);
}
